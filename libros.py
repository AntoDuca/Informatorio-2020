...
class conjuntoLibros:
    ...
    def __eliminarLibros_porTitulo(self, titulo):
        for x in self.listaDeLibros: #Para el elemento x en la lista de libros
            if (titulo == x.titulo): #si el título es igual al título del elemento x
                self.listaDeLibros.remove(x) #Remover ese elemento x
...
def crearLibro(titulo, autor, nro_paginas, calificacion):
    libro = Libro(titulo, autor, nro_paginas, calificacion)   #Así el usuario carga los libros que desea
    return libro
