#1############################################################################
"Solicite al usuario el ingreso de 3 números, e imprímalos de mayor a menor"

"""val1 = int(input("A continuación, ingrese 3 números: "))
val2 = int(input())
val3 = int(input())

print ("Los valores ingresados son: ", val1, val2, val3)

Min = min(val1, val2, val3)
Max = max(val1, val2, val3)
intermedio = (val1 + val2 + val3) - Min - Max

print("El numero más grande es: ", Max," el numero menor es: ", Min, " y el numero intermedio es: ", intermedio)"""

#2###########################################################################
"Desarrolle un programa que permita determinar si un número X ingresado es par o impar."

"""n = int(input("ingresa un numero: "))
resultado = (n % 2)
if resultado == 0:
    print("El numero ", n, "es par")
else :
    print("El numero ", n,  "es impar")"""

#3###########################################################################
"Determinar si el primero de un conjunto de tres números dados, es menor que los otros dos."

"""cactus = int(input("Bienvenido, por favor, ingresa 3 números: "))
cactus1 = int(input())
cactus2 = int(input())

aVer = min(cactus, cactus1, cactus2)
if aVer == cactus:
    print("El primer numero ingresado, ", cactus," es el menor")
else:
    print("El primer numero ingresado,",cactus," no es el menor,", " el numero menor es", aVer)
"""
#4###########################################################################
"Realizar un programa que sea capaz de, habiéndose ingresado dos números m y n, determine si n es divisor de m."
"""
def divisor(a, b):
    if (a % b == 0) :
        return("Si es divisor")
    else :
        return("No es divisor")

print(divisor(40,3))
"""
#5#PREGUNTAR#
"""
Diseñar un programa que lea las longitudes de los tres lados de un triángulo (L1,L2,L3)
y determine qué tipo de triángulo es, de acuerdo a los siguientes casos:
 Suponiendo que A determina el mayor de los tres lados y B y C corresponden con los otros dos, entonces:

Si A>=B + C à No se trata de un triángulo

Si A2 = B2 + C2 à Es un triángulo rectángulo

Si A2 > B2 + C2 à Es un triángulo obtusángulo

Si A2 < B2 + C2 à Es un triángulo acutángulo
"""


"""def esTriangulo (L1, L2, L3):
    if L1 >= L2 + L3:
        return("No es un triángulo")
    elif L1**2 == (L2**2 + L3**2):
        return ("Es un triángulo rectángulo")
    elif L1**2 > (L2**2 + L3**2) :
        return("Es un triángulo obtusángulo")
    else:
        return("Es un triángulo acutángulo")

print(esTriangulo(10, 3,7))
print(esTriangulo(10, 4,5))
print(esTriangulo(10, 5,5))
print(esTriangulo(100, 5,5))
print(esTriangulo(10, 20,13))"""

#6#RESUELTOOOOOOO##############################################################
"""
Un equipo de fútbol ha tenido una buena campaña y desea premiar a sus jugadores 
con un aumento del salario para la siguiente campaña. Los sueldos deben ajustarse de la siguiente forma:

Sueldo Actual (en $)    Aumento

0 – 6000					15%

6000 – 7900				   10%

7900 – 12000				6%

Más de 12000			   0%

Diseñar un programa que lea el salario de un jugador, y que a continuación muestre el tanto por ciento de aumento, 
el sueldo actual y el sueldo aumentado.


def aumento(salario_actual):
    if salario_actual <= 6000 :
        return (salario_actual * 0.15) + salario_actual
    elif salario_actual >= 6000 and salario_actual <= 7900 :
        return (salario_actual * 0.10) + salario_actual
    elif salario_actual > 7900 and salario_actual <= 12000 :
        return (salario_actual * 0.06) + salario_actual
    elif salario_actual > 12000 :
        return 'Sin aumento'
    else:
        "Datos ingresados incorrectos"

dato = int(input('Ingrese su salario: \n'))
dato = aumento(dato)

def porcentajeAumento (salario_actual):
    if salario_actual <= 6000 :
        return '15%'
    elif salario_actual >= 6000 and salario_actual <= 7900 :
        return '10%'
    elif salario_actual > 7900 and salario_actual <= 12000 :
        return '6%'
    elif salario_actual > 12000 :
        return 'Sin aumento'
    else:
        "Datos ingresados incorrectos"

porc = porcentajeAumento(dato)

print("Su aumento es del ", porc ," Nuevo salario:", dato)
"""
#7###########################################################################
"""Un comercio ofrece un descuento del 15% sobre el total de la compra si esta supera los $1000. 
Obtenga para determinado cliente cuánto deberá pagar finalmente por su compra y el descuento obtenido, si es que corresponde.

def aplica_descuento (valorDeCompra):
    if valorDeCompra > 1000 :
        return "Aplicar descuento del 15% - Total: $", valorDeCompra - valorDeCompra*0.15 
    else:
        return "No aplicar descuento"

print(aplica_descuento(300))"""

#8###########################################################################

"""Calcular el número de pulsaciones que una persona debe tener por cada 10 segundos de ejercicio, 
si la fórmula es: Número de pulsaciones = (220 - edad)/10"""

"""def num_pulsaciones(edad):
    return (220 - edad) / 10

def edadMasCinco (edad):
    return(edad + 5)

edad = int(input("Ingrese su edad: "))

print ("Sus pulsaciones deberían ser: ", num_pulsaciones(edad), "y si a su edad le sumamos 5, quedaría: ", edadMasCinco(edad))
"""
#9###########################################################################


"""En un Centro Asistencial existen tres áreas: Pediatría, Traumatología y Kinesiología. 
El presupuesto anual del hospital se reparte conforme a la sig. tabla:

ÁREA 	            	PORCENTAJE

Pediatría					60%

Traumatología		        20%

Kinesiología	    		 20%


Obtener la cantidad de dinero que recibirá cada área, para cualquier monto presupuestal que se ingrese por teclado.
"""


"""def presupuestoSegunArea (dinero,area):
    if area == "a":
        return  dinero * 0.20
    elif area == "b":
        return  dinero * 0.60
    elif area == "c":
        return  dinero * 0.20

area = str(input("Ingrese la letra que corresponde al area que desea consultar:\na_Pediatría\nb_Traumatología\nc_Kinesiología\n"))

print(presupuestoSegunArea(6000, area))"""

#10##########################################################################


Tres personas deciden invertir su dinero para fundar una empresa. Cada una de ellas invierte una cantidad distinta.
 Obtener el porcentaje que cada quien invierte con respecto a la cantidad total invertida.

persona1 = 30000
persona2 = 50000
persona3 = 80000






#11##########################################################################

"""Determinar si un alumno aprueba a reprueba un curso, sabiendo que aprobara si su promedio de tres calificaciones es mayor o igual a 70; 
desaprueba en caso contrario.

#12##########################################################################

Hacer un programa que imprima el nombre de un artículo, clave, precio original y su precio con descuento. 
El descuento lo hace en base a la clave, si la clave es 01 el descuento es del 10% y si la clave es 02 el descuento 
en del 20% (solo existen dos claves).

#13##########################################################################

En un supermercado se hace una promoción, mediante la cual el cliente obtiene un descuento dependiendo de un número que se escoge al azar. 
Si el numero escogido es menor que 74 el descuento es del 15% sobre el total de la compra, si es mayor o igual a 74 el descuento es del 20%. 
Obtener cuánto dinero se le descuenta.

#14##########################################################################

Leer 2 números; si son iguales que los multiplique, si el primero es mayor que el segundo que los reste y si no que los sume.

#15##########################################################################

Un obrero necesita calcular su salario semanal, el cual se obtiene de la siguiente manera:

i. Si trabaja 40 horas o menos se le paga $16 por hora

ii. Si trabaja más de 40 horas se le paga $16 por cada una de las primeras 40 horas y $20 por cada hora extra.

#16##########################################################################

Hacer un programa que calcule el total a pagar por la compra de camisas. Si se compran tres camisas o mas se aplica un descuento del 20% 
sobre el total de la compra y si son menos de tres camisas un descuento del 10%.

#17##########################################################################

Determinar y exhibir si la estatura de una persona adulta dada, es mayor que la estatura media de las personas adultas de su sexo, siendo:

- estatura media de mujeres adultas: 1,65 m.

- estatura media de varones adultos: 1,72 m.

#18##########################################################################

Se leen tres números que son las longitudes de los lados de un triángulo. 
Determinar e informar si el mismo es equilátero (3 lados iguales), isósceles (2 lados iguales) o escaleno (3 lados distintos).

#19##########################################################################

Una distribuidora de libros vende a librerías y a particulares. Aplica bonificaciones por cantidad según el siguiente criterio:

              i.      a librerías: hasta 24 unidades, el 20%; más de 24 unidades, el 25%.

              ii.      a particulares: menos de 6 unidades, nada; desde 6 hasta 18 unidades, el 5% y más de 18 unidades, el 10%.

El tipo de cliente está codificado así: 'L' para librerías, 'P' para particular. Dado el importe bruto de una compra de libros, 
el tipo de cliente de que se trata y la cantidad total pedida por el mismo, determinar el importe bruto bonificado.



"""